...
// Command Port Bits
#define D_I					PC3		// D/I Bit Number
#define R_W					PC4		// R/W Bit Number
#define EN					PC5		// EN Bit Number
#define CSEL1				PC6		// CS1 Bit Number
#define CSEL2				PC7		// CS2 Bit Number
...
typedef uint8_t* Image_t; // a glcd format bitmap (includes width & height)
typedef uint8_t* ImageXBM_t; // a "xbm" format bitmap (includes width & height)
...
void ks0108DrawBitmap(Image_t bitmap, uint8_t x, uint8_t y, uint8_t color);
void ks0108BitmapXBM(ImageXBM_t bitmapxbm, uint8_t x, uint8_t y, uint8_t color);

void ks0108LoadBitmap(unsigned char* bitmap);
